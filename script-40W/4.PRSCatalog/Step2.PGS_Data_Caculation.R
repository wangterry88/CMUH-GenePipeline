...
for (j in 1:length(PGS_NUM)){
  folder<-dir.create(paste0("./output/",PGS_NUM[j]))
  sub_folder<-dir.create(paste0("./output/",PGS_NUM[j],"/plot/"))
}
...
# Plot data output
  tmp_PRS.pheno.plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.plot-data.txt',collapse = '')
...
########## Distribution plot ##########
  ...
  tmp_dist_plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.distribution.png',collapse = '')
...
########## Plotting Coefficients on Odds Scale ##########
  ...
  tmp_out_or<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.OR-data.txt')
  ...
  tmp_or_plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.OR-Plot.png',collapse = '')
...
########## Quantiles plot ##########
  ...
  tmp_quant_plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.quantiles.png',collapse = '')
...
# Two sample T-test
  ...
  cat('\n')
  cat('\n')
  tmp_t_test<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.T-test.txt',collapse = '')
...
# Wilcoxon Whitney U test , specify alternative="less"
  ...
  cat('\n')
  cat('\n')
  tmp_u_test<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.U-test.txt',collapse = '')
...
# Wasserstein distence test
  ...
  cat('\n')
  tmp_Wass_test<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.Wasserstein-test.txt',collapse = '')
...
##### Train Test data output #####
  ...
  tmp.tarin.data<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.AUC.train-data.txt',collapse = '')
...
################# Table 1 of PRS Sample in GLM model (Dropped NAs) #################
  ...
  tmp_PRS_table1<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.table1.html',collapse = '')
...
######################## Prevalence plot and Table ################################
  ...
  tmp_PrevalencePlot_data<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.Prevalence-data.txt')
  fwrite(PrevalencePlot_data,tmp_PrevalencePlot_data,sep="\t",col.names = T)


  tmp_Prevalence_plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.Prevalence.png')
...
######################## Cumulative Risk plot and Table ################################
    ...
    tmp_risk_data<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.Cumulative_Risk-data.txt')
    fwrite(risk_data,tmp_risk_data,sep="\t",col.names = T,row.names=F)

    tmp_risk_plot<-paste0('./output/',PGS_NUM[j],'/plot/',PGS_NUM[j],'.Cumulative_Risk.png')
